...
(def excel-cell-position-pattern (re-pattern "^([A-Z]+)"))
(defn get-column-as-keyword [^String cellname]
  (keyword (second (re-find excel-cell-position-pattern cellname))))
  ...
  (let [value (atom "")
    ...
    (proxy [DefaultHandler]
      ...
      (endElement [uri localName name]
          ;(reset! last (.getEntryAt sst (Integer/parseInt @last))))
        (case name
...
(def ad0 (fnil + 0 0))
(def PRICE {:Toner 134.79, :Büroklammern 1.24, :Stift 3.92, :Druckerpapier 4.53, (keyword "Whiteboard Marker") 7.67, :Ordner 1.99})
...
(defn -main
  ...
  [& args]
  (println "Start")
  (time (load-wb "order_overview.xlsx" "order_output.xlsx"))
  (println "Finish"))
